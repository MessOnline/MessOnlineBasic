...
if (isset($_POST['submit'])) {
	...
	if (empty($name) || empty($uemail) || empty($passw1) || empty($passw2) || empty($mobile)) {
		...
		header("Location: ../register.html?signup=invalid name");
		exit();
	} elseif (!filter_var($uemail, FILTER_VALIDATE_EMAIL)) {
		header("Location: ../register.html?signup=invalid email");
		exit();
	} else {
		...
		if ($resultCheck > 0) {
			header("Location: ../register.html?signup=Email already exists");
			exit();
		} elseif ($passw1!=$passw2) {
			header("Location: ../register.html?signup=Password do not match");
			exit();
		} else {
			//Hashing Password
